...
class jenisbphtb  extends CI_Controller {
	...
	public function index()
	{
		if($this->input->is_ajax_request() and $this->input->post("action") and $this->input->post("param")) {
			...
			$this->api->setParam(array("Target" => "taxjenisssb", "Activity" => "get"));
			$data["ssb"] = $this->api->execute("object");
	
			$this->load->view('pajak/jenisbphtb/index', $data);
		}	
		
		
	}

	public function cetakjenisbphtb(){

		$data = $this->input->post();

		$arr = array("Target" 	=> "taxjenisssb", 
		   "Activity"	=> "get",
		   	"Limit" => "20");
		...
		$output["ssb"] = $this->api->execute("object");

		$this->load->view("pajak/jenisbphtb/pdf/cetak",$output);
	...
	protected function show_table(){
		...
		if(!empty($this->param["kywd"])) {
			$flt = $this->param["kywd"];
			$filter .= "(kode LIKE '%{$flt}%' or nama LIKE '%{$flt}%')";
		}
		...
		if(!empty($this->param["filter"]["nama"])) {
			$flt = "nama = '". $this->param["filter"]["nama"] ."'";
			$filter .= (empty($filter)) ? $flt : " and ". $flt;
		}
