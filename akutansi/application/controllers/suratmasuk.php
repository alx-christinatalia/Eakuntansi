...
class suratmasuk extends CI_Controller {
	...
	public function index()
	{
		if($this->input->is_ajax_request() and $this->input->post("action") and $this->input->post("param")) {
			...
			$this->load->view('agendasurat/suratmasuk/index');
	...
	public function edit($id)
	{
		if(!empty($this->input->post("param")))
		{
			...
			$log = $this->DoList->user_log("notarisagendasurat","Edit Surat Masuk",$_id,"No Surat ".$value['no_surat']);
		...
		}else
		{
			...
			$data["suratmasuk"] = $this->api->execute("object");
			$this->load->view("agendasurat/suratmasuk/edit",$data);	
		}
	...
	{
		$this->load->view('agendasurat/suratmasuk/tambah');
	}
	...
	public function delete($id){
		...
		$data = $this->api->execute("array");
		$log = $this->DoList->user_log("notarisagendasurat","Hapus Surat Masuk",$id,"Surat ID ".$id);
		redirect("suratmasuk");
	...
	public function simpan()
	{
		...
		$data = $this->api->execute("array");
		$log = $this->DoList->user_log("notarisagendasurat","Tambah Surat Masuk",$data->Output,"No Surat ".$value['no_surat']);
				...
				foreach($output->Data as $result) { 
					...
					if ($result->jenis_agenda == 1 ){
						...
						$return .= "<tr id='{$result->_id}'>";
							$return .= '<td>
											<div class="btn-group">
												...
												<ul class="dropdown-menu">
													<li><a role="button" href="'.base_url("suratmasuk/edit/".$result->_id).'">
														 Edit </a></li>
							...
							$return .= "<td>{$result->asal_surat}</td>";
							$return .= "<td>{$result->disposisi}</td>";
						$return .= "</tr>";
